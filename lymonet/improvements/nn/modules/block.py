...
# from ultralytics.nn.CA import CoordAtt
# from ultralytics.nn.MHSA import MHSA
# from ultralytics.nn.SWIMT import SwinTransformerBlock
from ..CA import CoordAtt
from ..MHSA import MHSA
from ..SWIMT import SwinTransformerBlock
...
class CA_Bottleneck(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, k=(3,3),e=0.5): #ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = Conv(c1, c_, k[1], 1, g=g)
        self.ca = CoordAtt(c_, c_, 32)
        self.add = shortcut and c1 == c2
...
class C2fCA(nn.Module):
    # C2f bottleneck with coordination attention
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c_ = int(c2 * e)
        self.cv1 = Conv(c1, 2 * self.c_, 1, 1)
        self.cv2 = Conv((2 + n) * self.c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(CA_Bottleneck(self.c_, self.c_, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C2fST(C2f):
    # C2f bottleneck with swim transformer block
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.c_ = int(c2 * e)
        num_heads = self.c_ // 32
        self.m = nn.Sequential(*(SwinTransformerBlock(self.c_, self.c_, num_heads, n) for _ in range(n)))
...
class MHSA(nn.Module):
    def __init__(self, n_dims, width=14, height=14, heads=4, pos_emb=False):
        # n_dims: 640, width: 14, height: 14, heads: 4, pos_emb: False
    ...
    def forward(self,  x):
        n_batch, C, width, height = x.size()  # C=640
        # 问，为什么要把C分成4份后，从[bs, 640, 20, 20]变成[bs, 4, 160, 400], 两个方向的特征变成了一维合理吗？
        ...
        # TODO: 问：这里为什么还要对attn进行一次permute？
...
class MHSA_Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = Conv(c_, c2, k[1], 1, g=g)
        self.mhsa = MHSA(c2, 14, 14 ,4 , False)  # 这里写死的14有点问题，不过不使用位置编码就没事了
        self.add = shortcut and c1 == c2

    def forward(self, x):
        # TODO：我感觉这个写法是错的，应该是self.cv2(self.(mhsa(self.cv1(x))))
        # return x + self.mhsa(self.cv2(self.cv1(x))) if self.add else self.mhsa(self.cv2(self.cv1(x)))
        return x + self.cv2(self.mhsa(self.cv1(x))) if self.add else self.cv2(self.mhsa(self.cv1(x)))
    
class C2f_MHSA2(nn.Module):
    """CSP Bottleneck with 2 convolutions and 1 MHSA. by csdn迪菲赫尔曼"""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        # c1: 1280, c2: 1280, n:3, shortcut:True
        super().__init__()
        self.c = int(c2 * e)  # hidden channels, 640
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        # self.m = nn.ModuleList(
            # MHSA_Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        # 为什么这的e用1.0?, k似乎应该写成k=(3, 3)
        self.m = nn.ModuleList(
            MHSA_Bottleneck(self.c, self.c, shortcut, g, k=(3, 3), e=1.0), Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0), Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0))
