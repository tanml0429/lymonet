...
def train(opt):
	...
	train_loader = datasets.StatusDataLoader(balance=opt.balance, dformat='places365').get_loader(
		trte_path=train_paths, batch_size=opt.batch_size, cfg=opt.cfg)
	val_loader = datasets.StatusDataLoader(balance=opt.balance, dformat='places365').get_loader(
	...
	optimizer = torch.optim.Adam(model.parameters(), lr=opt.cfg['lr0'], betas=(0.9, 0.999), eps=1e-08, weight_decay=opt.cfg['weight_decay'])
	...
	if opt.resume is None:
		...
		model, start_epoch = general.load_resume3(
		opt.resume, model)
	...
	for epoch in range(start_epoch, opt.epochs):
		...
		for i, (input, target, img_path, status) in enumerate(train_loader):
			...
			head, P, R, F1, acc = dm.general.confusion2score(confusion_matrix, round=5)
			...
			train_ret += f"mean_F1: {F1[-1]*100:>6.3f}% mean_acc: {acc[-1]*100:>6.3f}% batch_time: {total_time/(i+1):.4f}s "
			print(train_ret, end='')
		...
		for i, (input, target, img_path, statuses) in enumerate(val_loader):
			...
			with torch.no_grad():
				...
				head, P, R, F1, acc = dm.general.confusion2score(confusion_matrix)
				val_ret = \
					f'\rValidation [{epoch+1:>3}/{opt.epochs}] [{i+1:>3}/{len(val_loader):>3}] {"":>14} ' \
					f'mean_F1: {F1[-1]*100:>6.3f}% mean_acc: {acc[-1]*100:>6.3f}% valid_time: {total_time:.4f}s'
				print(val_ret, end='') 
		# val_ret = f'{val_ret} '
		# print(val_ret)
...
if __name__ == '__main__':
	...
	parser.add_argument('--cfg_file', type=str, default='/home/tml/VSProjects/LymoNet/lymonet/ResNet50/data/lymo.yaml', help='hyperparamers path')
	...
	parser.add_argument('--device', type=str, default='2', help='cuda device, i.e. 0,1,2,3 or cpu')
	...
	opt.source = '/data/tml/lymonet/lymo_yolo_square1'
