...
class StatusDataLoader(object):
	def __init__(self, balance=True, dformat='places365'):
...
class StatusDatasets(object):
	...
	"""
	def __init__(self, path, cfg, balace=True, dformat='places365'):
	...
	def __getitem__(self, idx, **kwargs):
		data = self.data
		img_path, status, img, target = self.get_one_data(data, idx, use_hue=True, **kwargs)
		img = np.array(img, dtype=np.float32)
		img = np.transpose(img / 255.0, (2, 0, 1))
		target = np.array(target, dtype=np.int64)
		return img, target, img_path, status
	
	def img_resize(self, img, fs, back_color=114):
		h, w = img.shape[:2]
		max_r = np.max([w / fs[0], h / fs[1]])
		rw, rh = int(w / max_r), int(h / max_r)
	
		resized_img = cv2.resize(img, (rw, rh))
		new_img = np.zeros((fs[1], fs[1], 3), dtype=np.uint8)
		new_img[...] = back_color  # 全部填充灰色114
		padx, pady = int((fs[0] - rw) / 2), int((fs[1] - rh) / 2)  
		new_img = dm.general.imgAdd(resized_img, new_img, x=pady, y=padx, alpha=1)
		return new_img

	def get_one_data(self, data, idx, use_hue=True, hue=.1, sat=1.5, val=1.5, **kwargs):
		...
		# exit()
		if img.shape[0] != fs[1] or img.shape[1] != fs[0]: 
			img = self.img_resize(img, fs)
	...
	def get_all_data(self, balance=False, dformat='places365'):
		"""
		根据路径和状态，获取样本。
